"""
Test script for weather_agent.

This script tests the weather_agent standalone to verify it works correctly.
"""
import asyncio
import json
...
async def test_weather_agent(destination: str):
    """
    Test the weather_agent with a given destination.
    
    Args:
        destination: The travel destination to search for weather information.
    """
    try:
        print(f"\n{'='*60}")
        print(f"Testing Weather Agent for: {destination}")
        print(f"{'='*60}\n")
        
        # Invoke the agent
        print("Invoking agent...")
        result = await weather_agent.ainvoke({
            "messages": [f"Find weather details for {destination}"]
        })
...
def main():
    ...
    # Run the async test
    result = asyncio.run(test_weather_agent(test_destination))
